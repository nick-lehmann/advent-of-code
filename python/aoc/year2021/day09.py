    ...
    def find_low_points(self, grid) -> List[Point]:
        ...
        for row in range(height):
            for column in range(width):
                ...
                if all(grid[row+x][column+y] > cell for x, y in d if 0 <= row+x < height and 0 <= column+y < width):
        ...
        while len(wave):
            ...
            for point in wave:
                for x, y in [(1,0), (0,1), (-1,0), (0,-1)]:
                    ...
                    other_point = Point(point.row+x, point.col+y, grid[point.row+x][point.col+y])
                    
                    if other_point in new_wave or other_point in basin:
                        continue
                    
                    if other_point.value > point.value and other_point.value != 9:
                        new_point = Point(point.row+x, point.col+y, grid[point.row+x][point.col+y]) 
                        new_wave += [new_point]
                        basin += [new_point]
