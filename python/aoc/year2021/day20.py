        ...
        for step in range(STEPS):
            ...
            for rowIndex in range(minx-1, maxx+2):
                for columnIndex in range(miny-1, maxy+2):
                    window = product(range(rowIndex-1,rowIndex+2), range(columnIndex-1,columnIndex+2))
                    enhancement_input = [0] * 9
                    
                    for i, (x,y) in enumerate(window):
                        if (x,y) in image or minx <= rowIndex <= maxx and miny <= columnIndex <= maxy and step % 2:
