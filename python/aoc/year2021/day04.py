...
class Board:
    ...
    def is_done(self) -> bool:
        return any(all(el.checked for el in row) for row in self.board) or any(
            all(el.checked for el in column) for column in zip(*self.board)
        )
    ...
    def score(self) -> int:
        return (
            sum(sum(el.num for el in row if not el.checked) for row in self.board)
            * self.last_number
        )
